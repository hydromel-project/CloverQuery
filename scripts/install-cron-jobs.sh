...
0 8 1 * * cd $PROJECT_DIR && $SCRIPT_DIR/run-monthly-reports.sh >> /var/log/clover-monthly.log 2>&1

# Daily sync without email at 6:00 AM
0 6 * * * cd $PROJECT_DIR && $SCRIPT_DIR/container-manager.sh sync-data >> /var/log/clover-sync.log 2>&1

# Health check every 30 minutes
*/30 * * * * cd $PROJECT_DIR && $SCRIPT_DIR/health-monitor.sh check >> /var/log/clover-health.log 2>&1

# Weekly cleanup on Sundays at 2:00 AM
0 2 * * 0 cd $PROJECT_DIR && $SCRIPT_DIR/container-manager.sh cleanup >> /var/log/clover-manager.log 2>&1
EOF
...
echo "Installed Jobs:"
echo "==============="
echo "1. Monthly Reports:  1st of month at 8:00 AM (with email)"
echo "2. Daily Data Sync:  Every day at 6:00 AM (no email)"
echo "3. Health Checks:    Every 30 minutes"
echo "4. Weekly Cleanup:   Sundays at 2:00 AM"
echo ""
...
echo "Daily Sync:      /var/log/clover-sync.log"
...
echo "sudo touch /var/log/clover-{monthly,sync,health,manager}.log"
